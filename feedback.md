...
- No input validation or error handling for non-integer or invalid inputs — Implement try-except blocks or validation checks to handle cases where the input is not a valid integer, providing clear error messages to the user.
- Missing user input handling as required by the assignment
- Incorrect output format; prints test results instead of exact strings 'Leap year' or 'Not a leap year'
- No input validation or error handling for non-integer or invalid inputs
...
- Implement try-except blocks or validation checks to handle cases where the input is not a valid integer, providing clear error messages to the user.
