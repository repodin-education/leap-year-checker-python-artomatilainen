...
- Lack of input validation and error handling for non-integer or invalid inputs — Implement try-except blocks or condition checks to handle cases where input is not a valid integer, providing clear error messages to the user.
- Missing user input handling as required by the assignment
- Incorrect output format; prints test results instead of exact strings 'Leap year' or 'Not a leap year'
- Lack of input validation and error handling for non-integer or invalid inputs
...
- Implement try-except blocks or condition checks to handle cases where input is not a valid integer, providing clear error messages to the user.
